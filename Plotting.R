...
aDefinitions = factor(
  c("PPT", "E_0-5", "E_5-15", "E_15-30", "E_>30"),
  levels = c("PPT", "E_0-5", "E_5-15", "E_15-30", "E_>30")
)

posYearlyA = data.frame(
  aDefinitions,
...
NPPobs = data.frame(Year = 1:nrow(ANPPandPrecip) + 1938,
                    NPP_obs = NPPobserved)
NPPmod = data.frame(
  Year = 1:nrow(ANPPandPrecip) + 1938,
  NPP_mod = posMu[, 1],
...
plot3 <- ggplot(NPPobs) +
  geom_line(data = NPPobs,
            aes(Year, NPP_obs),
            color = 'steelblue',
            size = 3) +
  geom_point(data = NPPobs,
             aes(Year, NPP_obs),
             color = 'steelblue',
             size = 3) +
