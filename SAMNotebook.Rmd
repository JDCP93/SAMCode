...
```{r Initial Model Run, eval=FALSE}
...
```{r Model run for all time blocks, eval=FALSE, include=TRUE}
# Source the function
...
```{r SpringBlock Model Runs, eval=FALSE}
source("SAMFunction.R")
...
plot1 <- ggplot(plotWeights,aes(x=Block,y=Weights,fill=Season)) +
  ...
  ggtitle("Weights for Spring/Non-spring Rainfall")
...
We plot a similar graph but solely for one model run. The below graph is for
a 5 year lag with a similar (but not the same) timeblock as Ogle et al.
This timeblock is of the form 12-12-6-6-2.

```{r 5 38 Plot}
# Load the data
load("SAM_posterior_5_38.RData")

# Assemble the dataframe
plotWeights = data.frame("Weights"=as.vector(t(SAM_posterior_5_38$monthlyWeights$mean)),
                         "YearIntoPast"=rep(0:4,each=12),
...
plot1 <- ggplot(plotWeights,aes(x=YearIntoPast,y=Weights,fill=Month)) +
  ...
  ggtitle("12-12-6-6-2 Weights")

last_plot()  

```
Noticeably here, there is less consistent emphasis on "spring" rainfall and 
some very peculiar results. Noticeably the extreme variations in weight seen in
the autumn of Year 2. It does not seem logical that rainfall received in the August 
and October of the previous year has a ~4x greater impact on NPP than that received
in July and September. It is the on/off nature of these weights that is most
concerning me. The other years look far more acceptable.



```{r 5 32 Plot}
# Load the data
load("SAM_posterior_5_32.RData")

# Assemble the dataframe
plotWeights = data.frame("Weights"=as.vector(t(SAM_posterior_5_32$monthlyWeights$mean)),
                         "YearIntoPast"=rep(0:4,each=12),
                         "Month"=rep(c("Dec","Nov","Oct","Sep", "Aug","Jul",
                                       "Jun", "May","Apr","Mar","Feb","Jan"),5))
# Assign the factors
...
plot1 <- ggplot(plotWeights,aes(x=YearIntoPast,y=Weights,fill=Month)) +
   geom_bar(stat="identity", position=position_dodge(), linetype = "solid",size=0.5,color="black") +
   facet_grid(.~YearIntoPast,scales = "free_x",switch = "x", space = "free_y") +
  ggtitle("12-6-6-4-4 Weights")

last_plot()  
```
The above plot shows that changing the timeblock can alter the respective 
weighting in each season. This is a 5year lag with 12-6-6-4-4. Notice how year 1
now has May-Jun as the key period whereas the above graph has this on a par with
Jan-Feb and Mar-Apr if we just averaged their weights.

```{r 3 6 plot}
# Load the data
load("SAM_posterior_3_6_2020-03-19.RData")

# Assemble the dataframe
plotWeights = data.frame("Weights"=as.vector(t(SAM_posterior_3_6$monthlyWeights$mean)),
...
plot1 <- ggplot(plotWeights,aes(x=YearIntoPast,y=Weights,fill=Month)) +
  ...
  ggtitle(paste0("3 year lag, spring/non-spring (R2 = ",
                 signif(SAM_posterior_3_6$R2,2), ", DIC = ",
                 signif(SAM_posterior_3_6$DIC,4), ", MAE = ",
                 signif(SAM_posterior_3_6$MAE,4),")"))

last_plot()  
```

The above model has the best DIC of all, is ranked in the 76th percentile or higher
for all performance metrics and is 5th overall when summing the ranks. It has
6 weights. 

```{r 4 30 Plot}
# Load the data
load("SAM_posterior_4_30.RData")

# Assemble the dataframe
plotWeights = data.frame("Weights"=as.vector(t(SAM_posterior_4_30$monthlyWeights$mean)),
                         "YearIntoPast"=rep(0:3,each=12),
                         "Month"=rep(c("Dec","Nov","Oct","Sep", "Aug","Jul",
                                       "Jun", "May","Apr","Mar","Feb","Jan"),4))
# Assign the factors
plotWeights$Month = factor(plotWeights$Month, levels = c("Jan","Feb","Mar","Apr",
                                                         "May","Jun","Jul","Aug",
                                                         "Sep","Oct","Nov","Dec"))
...
plot1 <- ggplot(plotWeights,aes(x=YearIntoPast,y=Weights,fill=Month)) +
   geom_bar(stat="identity", position=position_dodge(), linetype = "solid",size=0.5,color="black") +
   facet_grid(.~YearIntoPast,scales = "free_x",switch = "x", space = "free_y") +
  ggtitle(paste0("4 year lag, 12-12-4-2 (R2 = ",signif(SAM_posterior_4_30$R2,2), 
                 ", DIC = ", signif(SAM_posterior_4_30$DIC,4),
                 ", MAE = ", signif(SAM_posterior_4_30$MAE,4),")"))

last_plot()  
```

The above is the best performing model with no metric lower than the 78th percentile
and the highest sum of all ranks. It has 5 times as many timeblocks.

*MAJOR RESULT*
The 3 spring/non-spring models that consider lags of 3 years or more are in the
top 8 models. They perform well in DIC and Q5/Q95 but suffer in error metrics 
and R2. 

```{r 3 6 4 30 Mod vs Obs}
...
NPPobs = data.frame(Year=1:nrow(ANPPandPrecip)+1938,
                    NPP_obs = NPPobserved)
NPPmod430 = data.frame(Year=1:nrow(ANPPandPrecip)+1938,
                    NPP_mod = SAM_posterior_4_30$NPPmod$mean,
                    NPP_modmin = SAM_posterior_4_30$NPPmod$min,
                    NPP_modmax = SAM_posterior_4_30$NPPmod$max)

NPPmod36 = data.frame(Year=1:nrow(ANPPandPrecip)+1938,
                    NPP_mod = SAM_posterior_3_6$NPPmod$mean,
                    NPP_modmin = SAM_posterior_3_6$NPPmod$min,
                    NPP_modmax = SAM_posterior_3_6$NPPmod$max)

plot3 <- ggplot(NPPobs) +
  geom_line(data=NPPobs,aes(Year,NPP_obs),color='black',size=2) +
  geom_point(data=NPPobs,aes(Year,NPP_obs),color='black',size=2,na.rm=TRUE) +
  geom_ribbon(data=NPPmod430, aes(x=Year, ymin=NPP_modmin, ymax=NPP_modmax), fill="blue", alpha=0.2) +
  geom_ribbon(data=NPPmod36, aes(x=Year, ymin=NPP_modmin, ymax=NPP_modmax), fill="red", alpha=0.2) +
  geom_line(data=NPPmod430,aes(Year,NPP_mod),color = "blue") +
  geom_line(data=NPPmod36,aes(Year,NPP_mod),color = "red") +
  theme_bw() +
  theme(axis.line=element_line(colour = "black"),
    panel.grid.major=element_blank(),
    panel.grid.minor=element_blank(),
    panel.border=element_blank(),
    panel.background=element_blank()) +
 ggtitle(paste("3-6 S/NS (red) vs 4-30 (blue) against Observed NPP"))
  

grid.arrange(plot3)
```

I want to test whether 3-6 works best when split into spring/non-spring as above
or whether this is just chance. As such, I will test 3-6 with 6 monthly blocks
for each of the years, and also with a 1-1-4 split. 

```{r 3year Test Runs, eval=FALSE}
source("SAMFunction.R")
  # Create the timeblock for each year being split in 6 monthly blocks
  timeblock=timeblocks(0,0,0,3,0)
  Nlag = timeblock$Nlag
  block = timeblock$block
  # For the timeblock created, run the model
  SAM("data/dataset2.csv","data/dataset3.csv",Nlag,block,prior=FALSE)
  ...
  # Now run the model for a 1-1-4 timeblock split
  Nlag = 3
  block = matrix(c(rep(1,12),rep(2,12),rep(c(3,4,5,6),each=3)), 
                    nrow=Nlag,ncol=12,byrow = TRUE)
  # For the timeblock created, run the model
  SAM("data/dataset2.csv","data/dataset3.csv",Nlag,block,prior=FALSE)
```

When we plot these results, it is clear that partitioning on spring time appears
to give better results. 

```{r 3 year comparisons}
# Load the data
load("SAM_posterior_3_6_2020-03-19.RData")
load("SAM_posterior_3_6_20200323_102421.RData")
load("SAM_posterior_3_6_20200323_103033.RData")
...
# Assemble the dataframe for the 2-2-2 6monthly blocks
plotWeights2 = data.frame("Weights"=as.vector(t(SAM_posterior_3_6_20200323_102421$monthlyWeights$mean)),
                         "YearIntoPast"=rep(0:2,each=12),
                         "Month"=rep(c("Dec","Nov","Oct","Sep", "Aug","Jul",
                                       "Jun", "May","Apr","Mar","Feb","Jan"),3))
# Assign the factors
plotWeights2$Month = factor(plotWeights2$Month, levels = c("Jan","Feb","Mar","Apr",
                                                         "May","Jun","Jul","Aug",
                                                         "Sep","Oct","Nov","Dec"))
# Create the plot for grid.arrange
plot2 <- ggplot(plotWeights2,aes(x=YearIntoPast,y=Weights,fill=Month)) +
      geom_bar(stat="identity", position=position_dodge(), linetype = "solid",size=0.5,color="black") +
      facet_grid(.~YearIntoPast,scales = "free_x",switch = "x", space = "free_y") +
      ggtitle(paste0("3 year lag, 6 month blocks  (R2 = ",
                     signif(SAM_posterior_3_6_20200323_102421$R2,2), ", DIC = ",
                     signif(SAM_posterior_3_6_20200323_102421$DIC,4), ", MAE = ",
                     signif(SAM_posterior_3_6_20200323_102421$MAE,4),")"))


# Assemble the dataframe for the 1-1-4 model
plotWeights3 = data.frame("Weights"=as.vector(t(SAM_posterior_3_6_20200323_103033$monthlyWeights$mean)),
                         "YearIntoPast"=rep(0:2,each=12),
                         "Month"=rep(c("Dec","Nov","Oct","Sep", "Aug","Jul",
                                       "Jun", "May","Apr","Mar","Feb","Jan"),3))
# Assign the factors
plotWeights3$Month = factor(plotWeights3$Month, levels = c("Jan","Feb","Mar","Apr",
                                                         "May","Jun","Jul","Aug",
                                                         "Sep","Oct","Nov","Dec"))
# Plot
plot3 <- ggplot(plotWeights3,aes(x=YearIntoPast,y=Weights,fill=Month)) +
      geom_bar(stat="identity", position=position_dodge(), linetype = "solid",size=0.5,color="black") +
      facet_grid(.~YearIntoPast,scales = "free_x",switch = "x", space = "free_y") +
      ggtitle(paste0("3 year lag, 1-1-4 block (R2 = ",
                     signif(SAM_posterior_3_6_20200323_103033$R2,2), ", DIC = ",
                     signif(SAM_posterior_3_6_20200323_103033$DIC,4), ", MAE = ",
                     signif(SAM_posterior_3_6_20200323_103033$MAE,4),")"))
